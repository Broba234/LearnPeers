    ...
    <div className="min-h-screen bg-[#FAFAF9]">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        {profile?.Institutions && (
          <div className="mb-6 flex items-center gap-3 bg-brand-50 rounded-xl px-4 py-3">
            ...
            <div>
              <p className="text-sm font-medium text-brand-900">{profile.Institutions.name}</p>
              <p className="text-xs text-brand-500">Showing courses from your institution</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${subjects.length >= 5 ? "bg-brand-50 text-brand-600" : "bg-slate-100 text-slate-500"}`}>
              {subjects.length} / 5
            </span>
          </div>
          <SubjectSelector
