          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {[9, 10, 11, 12].map(grade => (
              <button key={grade} type="button"
                className={`px-3 py-1.5 rounded-full border text-xs font-medium transition-all ${gradeFilter === grade ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"}`}
                        ...
                        >
                          <span className="truncate max-w-[200px]">
                            {item.institution_code
                              ? <><span className="font-mono font-semibold">{item.institution_code}</span> <span className={isSelected ? "text-indigo-200" : "text-slate-400"}>· {item.name}</span></>
                              : <>{item.name}{item.code && <span className={`ml-1 ${isSelected ? "text-indigo-200" : "text-slate-400"}`}>({item.code}){item.grade ? ` · ${gradeLabel(item.grade)}` : ""}</span>}</>
                            }
                          </span>
                          {isSelected && (
