...
>
> **2026-09-15 status check (daily routine):** no commits landed since `098c5cd` (2026-09-12), so this is a re-verification pass, not a new-fix pass. **§11.8** is fixed — `contacts/page.tsx` already keys the `<Fragment>` wrapping each row pair (`<Fragment key={contact.id}>`), not the inner `<tr>`; nothing left to do here, closing this row. **§8.6**'s `TutorCard.tsx` no longer matches the audited description at all — the component has been rebuilt since with a `saved`/`onToggleSave` prop pair (not the audited dead `onBook`), rating/review-count/session-count proof row, a "grade verified" headline banner, and live-slots-today badge; the specific dead props this row named are gone. The rest of §8.6/§8.7/§8.8 (the broader "long tail" across `AddSubject`, `SetupWizard`, `EventModal`, `FilterModal`, `WizardTimeSlot`, `EventDetailModal`, unused UI primitives) wasn't re-swept file-by-file today — treat those sub-items as unverified rather than confirmed-open, since at least one named file in the same rows has already drifted out of date. Re-confirmed still genuinely open, unchanged since 2026-09-12: **§6.1** `strict: false` (`tsconfig.json:11` + the four related strict flags), **§6.2** CSP still ships `'unsafe-eval' 'unsafe-inline'` on `script-src` (`next.config.js:18`), and **§11.1** `prisma/migrations/` still ends at `20260708120000_saved_payment_methods` while `prisma/sql/*.sql` carries hand-run changes past that point — migration history and schema still don't agree on a fresh DB. None of these three are safe as an unattended drive-by fix; each needs a dedicated, reviewed pass as previously noted.
>
> **2026-09-16 status check (daily routine):** re-verified the earlier "already fixed" rows named in this table (§4.3, §7.1/7.2, §8.3, §8.4, §8.5, §9.4) — all still hold (`dotenv` stayed in `devDependencies`, no `catch {}` blocks, no `app/api/booking/create`, `totalEarnings` is rendered not discarded, `handleCreateSubject` is gone). Two new things landed today: **§11.11**'s `admin/login/page.tsx:62` silent-loop bug — a non-admin signing in there fell through to a `default: homePath = "/admin/login"` case and got bounced back to the same page with zero explanation; replaced with an explicit "admin accounts only" error message (the other two `SignUpWizard.tsx`/`student/page.tsx` items in that row are still open). Also ran `npm audit fix`, which patches the **critical** Next.js RCE advisory (bumped to 16.3.5) plus `sharp`/`js-yaml`, all within the existing `package.json` semver ranges — no code changes needed. 8 findings remain, all the same Excalidraw→mermaid→langium→chevrotain→`lodash-es` transitive chain noted in past passes; fixing that still requires `--force` (a major Excalidraw bump) and wasn't attempted as a drive-by. **§6.1**, **§6.2**, and **§11.1** remain open and still need a dedicated pass, unchanged from 2026-09-12/09-15.
>
> **2026-09-18 status check (daily routine):** no commits landed 09-17 (routine didn't run / nothing to do). Closed the remaining two **§11.11** items: `SignUpWizard.tsx`'s `handleInputChange` simplified `(value == 1) === true || value === 1` down to `value == 1` (same behavior, no longer nonsense); `app/home/student/page.tsx`'s role-mismatch check called `router.push("/auth/login")` directly in the render body without returning, so a non-student briefly fell through and rendered the full student dashboard (with a real API call for that student's sessions) before the redirect took effect — moved the push into a `useEffect` keyed on `profile` and made the render path return a loader immediately on mismatch, matching the pattern the rest of the component already uses for its other guard clauses. Re-swept §7.1/7.2 (still 0 empty `catch {}` blocks), §8.3 (still no `booking/create` route), §10.3 (no stray placeholder comments in the four files this row named), and §11.7 (the SQL header already cites the real `prisma db execute` command) — all still closed. Verified with `tsc --noEmit` (clean) and `eslint .` (0 errors, same pre-existing warning set as before, no new warnings on the two changed files). `next build` could not be exercised in this sandbox — no `.env.local`/Supabase credentials are available here, and the prerender step fails on `/auth/login` and `/admin/login` for that reason on a clean checkout too (confirmed by stashing these changes and re-running the build), so this isn't a regression from today's edit; re-run `next build` with real env vars before the next deploy to get a true green build signal. §6.1, §6.2, and §11.1 remain open, unchanged — still not safe as an unattended drive-by.
>
> **2026-09-19 status check (daily routine):** re-verified the small/tractable still-open rows named for today's pass. **§7.3** and the `student/sessions/page.tsx` part of **§8.6** are stale — that page has been rebuilt since the audit (no empty `else` branches, no unused `errorData`, no dead modal scaffolding; `Modal`/`Button` are both genuinely used), so nothing to fix there; closing both. The `tutor/availability/page.tsx` and `explore/components/TutorCard.tsx` parts of §8.6 were already confirmed stale on 2026-09-15 and are unchanged. **§8.5**/**§10.1** were already fixed by a prior commit (`app/(admin)/dashboard/subjects/page.tsx` is 515 lines, no commented-out "Create New Subject" form, no `handleCreateSubject`) — the 2026-09-12 entry noted this but the table rows were never flipped to ✅; fixed now. **§8.7** is genuinely still open: `npx eslint` on the nine named files/areas (`AddSubject.tsx`, `SetupWizard.tsx`, `EventModal.tsx`, `FilterModal.tsx`, `WizardTimeSlot.tsx`, `EventDetailModal.tsx`, `tutor/[id]/page.tsx`, `admin/login/page.tsx`, `approvals/page.tsx`) shows a real long tail of unused imports/vars (e.g. `AddSubject.tsx`'s unused `Subject`/`CategoryOption`/`GradeOption`/`router`/`setLoading`, `SetupWizard.tsx`'s unused `Upload`/`Globe`/`Phone`/`PrinterCheck` icons, `FilterModal.tsx`'s unused `gradeLabel`/`GRADES`) — too broad for one drive-by, left open. **§10.2** is also genuinely still open but only in `SignUpWizard.tsx` (`components/student/SignUpWizard.tsx:298`, a step row with `cursor-pointer` in its className and a commented-out `onClick={() => setActiveStep(step.number)}` right above it — looks clickable, does nothing — plus two small dead commented-JSX fragments at lines 309 and 393-395); `SetupWizard.tsx`'s copy of the same pattern was already fixed by an earlier commit (no commented `onClick`/dead JSX left there) even though the row still names both files — left open pending a dedicated pass since it wasn't today's pick. **§11.9 was miscalled "already fixed" on 2026-09-03** — re-verified today and it's still broken: `curricula`, `institutions`, `feedback`, `approvals`, `contacts`, and `subjects` under `app/(admin)/dashboard/` all still wrapped their content in `<div className="max-w-7xl mx-auto">` nested inside `app/(admin)/layout.tsx`'s own `max-w-[1600px] mx-auto` wrapper (the newer dashboard pages — `growth`, `financials`, `marketplace`, `quality`, `access`, `onboarding`, `users` — already use the correct bare-`<div>` pattern), and `subjects/page.tsx`'s loading state used an off-brand `bg-gradient-to-b from-[#F8F9FD] to-gray-400` box instead of the `bg-brand-50 dark:bg-brand-900/20` pattern every sibling page uses. **Fixed today**: stripped the redundant `max-w-7xl mx-auto` className from all 6 pages (now bare `<div>`, matching the layout's own constraint and the newer pages' pattern) and swapped the subjects loading box to the shared `bg-brand-50`/`border-brand-200` styling. §11.10 re-confirmed fine (`public/default-avatar.png` exists, per 2026-09-03). Verified with `tsc --noEmit` (clean) and `npm run lint` (0 errors, 396 pre-existing warnings, no new warnings on any of the 6 touched files — `curricula`/`institutions` show only pre-existing `any` warnings on unrelated lines, `subjects` only a pre-existing unused-`err` warning, `feedback`/`approvals`/`contacts` show none). Could not exercise `next dev`/`next build` end-to-end for these admin pages in this sandbox — no Supabase env vars are set, so `app/(admin)/layout.tsx`'s `getAdminContext()` throws before rendering any admin page (`Error: Your project's URL and Key are required to create a Supabase client!`), confirmed by curling `/dashboard/curricula` and `/dashboard/subjects` under `next dev` (both 500, same Supabase-client error, unrelated to today's className-only edit); this is the same known pre-existing sandbox limitation noted on 2026-09-18, not a regression. §6.1, §6.2, and §11.1 remain open, unchanged — still not safe as an unattended drive-by.
>
> Also found and fixed a new one, not previously tracked in this table: both empty states on `app/home/student/explore/page.tsx` ("Be first in line" and the no-search-results state) linked to `/#contact`, but no page anywhere defines that anchor, and nothing in the app ever `fetch`-called the existing, working, public `POST /api/contacts` — so "Request a tutor" went nowhere and an unmatched course search silently dropped real unmet-demand signal, which matters a lot for a two-sided marketplace still building out tutor supply. Added `RequestCourseModal` (existing design-system primitives, wired to `/api/contacts`) and opened it from both empty states, prefilled with the searched course when there is one. Verified with `tsc --noEmit` + `eslint .` (clean, no new warnings) and a screenshot of the rendered modal (env-var-free sandbox couldn't exercise the full authenticated page, so the modal was rendered in isolation instead).
...
| 7.3 | ⚪ | `student/sessions/page.tsx:58,60,103` | Empty `else` branches; `errorData` parsed then never used. | ✅ Fixed (page rebuilt since audit; no empty branches, no unused `errorData` — confirmed 2026-09-19) |
...
| 8.5 | `app/(admin)/dashboard/subjects/page.tsx:35-50,165-218` | `handleCreateSubject` + state reachable only from a commented-out form (see 10.1). | ✅ Fixed (gone by 2026-09-12 per Appendix; table row never updated — confirmed 2026-09-19) |
| 8.6 | `app/home/student/sessions/page.tsx:30-35`, `tutor/availability/page.tsx:6-7`, `explore/components/TutorCard.tsx:11,28` | Dead modal scaffolding, duplicate import, dead `onBook` prop + `tzTime` (orphans `currentTimeInTz`). | ✅ Fixed (all 3 named files rebuilt since audit — `TutorCard`/`availability` confirmed 2026-09-15, `student/sessions` confirmed 2026-09-19) |
...
| # | Location | Issue | Status |
|---|---|---|---|
| 10.1 | `app/(admin)/dashboard/subjects/page.tsx:585-676` | ~90-line commented-out "Create New Subject" form (live page uses `<AddSubject>`). | ✅ Fixed (gone by 2026-09-12 per Appendix; table row never updated — confirmed 2026-09-19) |
...
| 11.9 | 6 admin ops pages | Double-wrapped layout (nested max-width/padding) + off-brand loading gradient. | ✅ Fixed (2026-09-19, daily routine — was wrongly marked fixed 2026-09-03, still broken; now actually fixed) |
...
| 11.11 | `SignUpWizard.tsx:134`, `student/page.tsx:180` | Convoluted boolean; redirect-during-render. `admin/login/page.tsx`'s silent non-admin login loop fixed 2026-09-16 (see below). | ✅ Fixed (2026-09-18, daily routine) |
